...
CATEGORIES = ["Apose", "Statue", "Bad", "Custom"]
...
class Watch:
    ...
    def run(self, gui):
        ...
        try:
            while gui.watchdog is not None:
                print("Watch running...")
                time.sleep(5)
            ...
            if os.path.exists(event.src_path):
                ...
                if dir_name != self.parent:
                    ...
                    destination = os.path.join(
                        self.dest, destination_subfolder, ticket_name
                    )
...
class GUI:
    ...
    def checkRunButtonState(self):
        if (
            self.var.get() == "None Selected"
            or self.drop.get() == "None Selected"
            or self.arch.get() == "None Selected"
        ):
    ...
    def copyFilesWithProgress(self, src, dest, thumbName, protocol="move"):
        ...
        for widget in self.pbarFrame.winfo_children():
            widget.destroy()
        p = ttk.Progressbar(
            self.pbarFrame, orient="horizontal", length=320, mode="determinate"
        )
        p.pack()
                ...
                if protocol == "copy":
                    ...
                    if self.thumbnail.get():
                        ...
                        if f"{thumIdx}.jpg" in sfile:
                            ...
                            img_rotate_90_clockwise = cv.rotate(
    ...
    def startWatch(self):
        if self.watchdog == None:
            ...
            self.runButton.config(style="redButton.TButton", text="Watching...")
            self.runButton.state(["disabled"])
    ...
    def __init__(self):
        ...
        self.leftFrame = Frame(
            self.root, borderwidth=2, relief="solid", width=400, height=600
        )
        self.leftFrame.config(background="lightgrey")
        self.leftFrame.grid(row=0, column=0, padx=10, pady=5)

        self.rightFrame = Frame(
            self.root, borderwidth=2, relief="solid", width=250, height=600
        )
        ...
        ttk.Progressbar(
            self.pbarFrame, orient="horizontal", length=500, mode="determinate"
        ).pack()
        ...
        self.dropButton = ttk.Button(
            self.leftFrame, text="Set Drop Folder", command=self.getDropFolder
        )
        ...
        self.archButton = ttk.Button(
            self.leftFrame, text="Set Archive Folder", command=self.getArchiveFolder
        )
        ...
        self.runButton = ttk.Button(
            self.leftFrame, text="Run Watcher", command=self.runWatcher
        )
